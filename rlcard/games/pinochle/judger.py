...
class PinochleJudger:

    '''
        Judger decides legal actions for current player
    '''

    def __init__(self, game: 'PinochleGame'):
        ''' Initialize the class PinochleJudger
        :param game: PinochleGame
        '''
        self.game: PinochleGame = game
        ...
        if not self.game.is_over():
            ...
            if not self.game.round.is_bidding_over():
                ...
                if trick_moves and len(trick_moves) < 4:
                    led_card: PinochleCard = trick_moves[0].card
